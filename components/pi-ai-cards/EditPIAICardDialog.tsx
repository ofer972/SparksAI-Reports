...
interface EditPIAICardDialogProps {
  card: PIAICard;
  onSave: (updatedCard: PIAICard) => Promise<void>;
  onClose: () => void;
}

// Initialize formData with all possible fields, using card values or defaults
const initializeFormData = (cardData: PIAICard): PIAICard => {
  return {
    id: cardData.id ?? 0,
    date: cardData.date ?? '',
    pi_name: cardData.pi_name ?? '',
...
export default function EditPIAICardDialog({
  card,
  onSave,
  onClose,
}: EditPIAICardDialogProps) {
  const initialFormData = useMemo(() => initializeFormData(card), [card]);
  const [formData, setFormData] = useState<PIAICard>(initialFormData);
  ...
  const handleChange = (field: keyof PIAICard, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updates: Partial<PIAICard> = {};
      allFields.forEach(field => {
        if (!readOnlyFields.includes(field)) {
          let value: any = formData[field as keyof PIAICard];
          ...
          const nullableFields = ['source_job_id', 'information_json'];
          if (value !== undefined) {
            // For nullable fields, include null if explicitly set
            if (nullableFields.includes(field)) {
              updates[field as keyof PIAICard] = value;
            }
  ...
  // Define all possible fields from the PIAICard interface to ensure all fields are shown
  const allPossibleFields: (keyof PIAICard)[] = [
    'id',
    'date',
    'pi_name',
    'card_name',
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] flex flex-col">
        <div className="border-b border-gray-200 bg-blue-50 px-4 py-2 rounded-t-lg">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-900">Edit PI AI Card</h3>
            <button
              onClick={onClose}
          ...
          <div className="px-4 py-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
              {allFields.map((field) => {
                ...
                const isShortField = ['id', 'date', 'pi_name', 'card_name', 'card_type', 'priority', 'source', 'source_job_id'].includes(field);
                  ...
                  <div key={field} className="flex items-center gap-2">
                    ...
                    {isDate ? (
                      <input
                        ...
                        onChange={(e) => handleChange(field as keyof PIAICard, e.target.value)}
                        disabled={isReadOnly}
                      ...
                      <input
                        ...
                        onChange={(e) => handleChange(field as keyof PIAICard, e.target.value ? Number(e.target.value) : null)}
                        disabled={isReadOnly}
                        className={`flex-1 px-2 py-1 text-xs border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 ${
                          isReadOnly ? 'bg-gray-100 cursor-not-allowed' : ''
                        }`}
