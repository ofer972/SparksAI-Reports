...
interface EditRecommendationDialogProps {
  ...
  onSave: (updatedRecommendation: Recommendation) => Promise<void>;
  onClose: () => void;
}

// Initialize formData with all possible fields, using recommendation values or defaults
const initializeFormData = (recommendationData: Recommendation): Recommendation => {
  return {
    id: recommendationData.id ?? 0,
    team_name: recommendationData.team_name ?? '',
    date: recommendationData.date ?? '',
    action_text: recommendationData.action_text ?? '',
    rational: recommendationData.rational ?? '',
    full_information: recommendationData.full_information ?? '',
    priority: recommendationData.priority ?? '',
    status: recommendationData.status ?? '',
    information_json: recommendationData.information_json ?? null,
    created_at: recommendationData.created_at,
    updated_at: recommendationData.updated_at,
  };
};
...
export default function EditRecommendationDialog({
  ...
  const initialFormData = useMemo(() => initializeFormData(recommendation), [recommendation]);
  const [formData, setFormData] = useState<Recommendation>(initialFormData);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setFormData(initializeFormData(recommendation));
  }, [recommendation]);
    ...
    try {
      ...
      allFields.forEach(field => {
        if (!readOnlyFields.includes(field)) {
          let value: any = formData[field as keyof Recommendation];
      ...
      // Create updated recommendation with all fields for the callback
      const updatedRecommendation: Recommendation = {
        ...recommendation,
        ...updates,
      };

      await onSave(updatedRecommendation);
    } catch (err) {
  ...
  // Define all possible fields from the Recommendation interface to ensure all fields are shown
  const allPossibleFields: (keyof Recommendation)[] = [
    'id',
    'team_name',
    'date',
    'action_text',
  ...
  // Use all possible fields, but merge with actual recommendation data
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] flex flex-col">
        <div className="border-b border-gray-200 bg-blue-50 px-4 py-2 rounded-t-lg">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-900">Edit Recommendation</h3>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="px-4 py-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
              {allFields.map((field) => {
                const fieldValue = formData[field] ?? recommendation[field] ?? null;
                const isTextArea = field === 'rational' || field === 'full_information' || field === 'information_json' || field === 'action_text';
                const isShortField = ['id', 'date', 'team_name', 'priority', 'status'].includes(field);
                  ...
                  <div key={field} className="flex items-center gap-2">
                    ...
                    {isDate ? (
                      <input
                        ...
                        onChange={(e) => handleChange(field as keyof Recommendation, e.target.value)}
                        disabled={isReadOnly}
                      ...
                      <input
                        ...
                        onChange={(e) => handleChange(field as keyof Recommendation, e.target.value ? Number(e.target.value) : null)}
                        disabled={isReadOnly}
                        className={`flex-1 px-2 py-1 text-xs border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 ${
                          isReadOnly ? 'bg-gray-100 cursor-not-allowed' : ''
                        }`}
            ...
            <div className="grid grid-cols-1 gap-3">
              {allFields.map((field) => {
                ...
                // Handle other fields that aren't short fields or textareas (like created_at, updated_at)
                if (!isTextArea) {
                  return (
                    <div key={field}>
                      <label
