...
import EditTeamAICardDialog from './EditTeamAICardDialog';
import Toast from '@/components/Toast';

export default function TeamAICardsPage() {
  const [data, setData] = useState<TeamAICard[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [sorting, setSorting] = useState<SortingState>([]);
  const [editingCard, setEditingCard] = useState<TeamAICard | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const result = await apiService.getTeamAICards(
        dateFilter || undefined,
        cardNameFilter || undefined
      );
      setData(result.cards || []);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch team AI cards';
      setError(errorMessage);
      setData([]);
      if (process.env.NODE_ENV === 'development') {
        console.error('Team AI Cards API Error:', {
          error: err,
          endpoint: '/api/v1/team-ai-cards',
          dateFilter,
          cardNameFilter
        });
      }
  ...
  const handleEdit = useCallback((card: TeamAICard) => {
    setEditingCard(card);
  }, []);

  const handleSave = useCallback(async (updatedCard: TeamAICard) => {
    try {
      setLoading(true);
      setError(null);
      // Only send updatable fields (exclude id, created_at, updated_at)
      const { id, created_at, updated_at, ...updates } = updatedCard;
      const result = await apiService.updateTeamAICard(id, updates);
      ...
      setEditingCard(null);
      setToast({ message: 'Successfully saved team AI card', type: 'success' });
      await fetchData(); // Refresh data
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to update team AI card';
      setError(errorMessage);
  ...
  const columns = useMemo<ColumnDef<TeamAICard>[]>(() => {
    if (data.length === 0) {
      return [];
    }
    ...
    const actionColumn: ColumnDef<TeamAICard> = {
      id: 'action',
      header: 'ACTION',
      size: 80,
      cell: ({ row }: { row: any }) => {
    ...
    const otherColumns = columnKeys.map((key) => {
      const value = firstItem[key as keyof TeamAICard];
      let cellRenderer;
      ...
      else if (key === 'team_name' || key === 'card_name' || key === 'card_type') columnSize = 150;
    ...
    <div className="h-full flex flex-col p-4">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex-1 flex flex-col overflow-hidden">
        <div className="p-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Team / Group AI Cards</h2>
        ...
        <div className="flex-1 overflow-auto" style={{ scrollbarWidth: 'thin', scrollbarColor: '#cbd5e0 #f7fafc' }}>
          <style jsx global>{`
            .team-ai-cards-table-container::-webkit-scrollbar {
              width: 12px;
              height: 12px;
            }
            .team-ai-cards-table-container::-webkit-scrollbar-track {
              background: #f7fafc;
            }
            .team-ai-cards-table-container::-webkit-scrollbar-thumb {
              background: #cbd5e0;
              border-radius: 6px;
            }
            .team-ai-cards-table-container::-webkit-scrollbar-thumb:hover {
              background: #a0aec0;
            }
          `}</style>
          {loading ? (
            ...
            <div className="overflow-x-auto overflow-y-auto team-ai-cards-table-container" style={{ maxHeight: 'calc(100vh - 300px)' }}>
